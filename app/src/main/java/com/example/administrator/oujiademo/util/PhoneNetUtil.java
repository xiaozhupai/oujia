...
import com.example.administrator.oujiademo.statusBar.PhoneNetLevel;
 ...
 * -140以下为无信号
 */
public class PhoneNetUtil implements PhoneNetLevel {
    ...
    private PhoneNetUtil(Context context) {
        getNetDBM(context.getApplicationContext());
    }
    ...
    private void setNetLevel(int dbm) {
        if (null != phoneNetLevelListener) {
            if (dbm > DBM_1) {
                phoneNetLevelListener.onNetLevel(BEST);
            } else if (DBM_2 < dbm && dbm < DBM_1) {
                phoneNetLevelListener.onNetLevel(BETTER);
            } else if (DBM_3 < dbm && dbm < DBM_2) {
                phoneNetLevelListener.onNetLevel(NORMAL);
            } else if (DBM_4 < dbm && dbm < DBM_3) {
                phoneNetLevelListener.onNetLevel(BAD);
            } else if (DBM_5 < dbm && dbm < DBM_4) {
                phoneNetLevelListener.onNetLevel(WORSE);
            } else {
                phoneNetLevelListener.onNetLevel(WORST);
            }
        }
    }
